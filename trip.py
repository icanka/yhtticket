            ...
            # we have already reserved the seat check lock_end_time and if it is passed then reserve the seat again
                ...
                if time_diff.total_seconds() < 5:
                    ...
                    lock_end_time, _, self.seat_lock_response = (
                        self.api.select_first_empty_seat(
        ...
        while len(trips_with_empty_seats) == 0:
            ...
            try:
                for trip in trips:
                    ...
                    logging.info("Checking trip for empty seats: %s", trip.get("binisTarih"))
                    ...
                    if empty_seat_count > 0:
                        logging.info("Found trip with empty seats.")
...
def list_stations():
    ...
    api = trip_search.TripSearchApi()
    stations_json = api.get_station_list()
    stations = []
